      ...
      <div className="mb-6">
        ...
        {playlist.tracks.length > 0 && filteredTracks.length > 0 ? (
                    ...
                    <div className="aspect-square bg-surface rounded-lg mb-2 overflow-hidden relative shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <div className={`absolute inset-0 flex items-center justify-center transition-opacity bg-black/40 ${isPlaying ? "opacity-100" : "opacity-0 group-hover/pl:opacity-100"}`}>
                        <div className="w-12 h-12 rounded-full bg-accent flex items-center justify-center shadow-lg">
                          {isPlaying ? <EqualizerIcon /> : <svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><polygon points="7,3 21,12 7,21" /></svg>}
                        </div>
                      </div>
                    </div>
                    <p className={`text-sm font-semibold leading-tight line-clamp-2 mb-1 ${isPlaying ? "text-accent" : "text-foreground"}`}>{track.title}</p>
          ...
          <div className="text-center py-16">
            ...
            <p className="text-text-secondary">No se encontraron resultados para &ldquo;{searchQuery}&rdquo;</p>
          </div>
        ) : (
          <div className="text-center py-16">
            <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="mx-auto mb-4 text-text-tertiary opacity-50">
              <path d="M9 18V5l12-2v13" /><circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" />
            </svg>
            <p className="text-text-secondary text-lg mb-2">Playlist vacía</p>
            <p className="text-text-tertiary text-sm">Busca canciones y agrégalas a esta playlist</p>
          </div>
        )}
    </div>
  );
